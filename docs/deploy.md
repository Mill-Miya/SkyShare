...
`VITE_GUEST_ACCESS_CODE` は部内テスト用の簡易メンテナンス認証です。公開URLおよび `/join/{sessionId}` では最初にメンテナンス認証画面を表示し、正しいパスワード入力後に通常画面またはGuest参加へ進みます。誤入力時は同じ認証画面に留まり、通信やGuest参加は開始しません。
